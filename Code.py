...
def list_vehicles():
    ...
    for idx, vehicle in enumerate(vehicles, start=1):
        print(f"{idx}. {vehicle['license_plate']} - {vehicle['make']} {vehicle['model']} ({vehicle['year']}) - {vehicle['status']}")
...
def remove_vehicle():
    ...
    if 0 <= vehicle_idx < len(vehicles):
        ...
        save_vehicles()
        print(f"Removed vehicle: {removed_vehicle['license_plate']} - {removed_vehicle['make']} {removed_vehicle['model']}")
    else:
